...
public class AccountControllerBootTests {
	...
	public void getAccounts() throws Exception {
		List<Account> accounts = List.of(
				new Account("8901234567", "Robertino Napoli"),
				new 		Account("1029384756", "Thom Yorke")
		);
		...
		mockMvc.perform(get("/accounts"))
				.andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("$[0].name").value("Robertino Napoli"))
				.andExpect(jsonPath("$[1].name").value("Thom Yorke"))
				.andExpect(jsonPath("$[0].number").value("8901234567"))
				.andExpect(jsonPath("$[1].number").value("1029384756"));
//				.andExpect(jsonPath("$.length()").value(Matchers.greaterThanOrEqualTo(21)));

		verify(accountManager).getAllAccounts();
	}

	@Test
	public void beneficiaryDetails() throws Exception {
		Account account = new Account("5645363735", "Crystal Methyd");
		account.addBeneficiary("Anouk Pancaldi", new Percentage(0.2));

		given(accountManager.getAccount(0L)).willReturn(account);

		mockMvc.perform(get("/accounts/{accountId}/beneficiaries/{beneficiaryId}", 0L, "Anouk Pancaldi"))
				.andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(jsonPath("name").value("Anouk Pancaldi"))
				.andExpect(jsonPath("allocationPercentage").value(0.2));

		verify(accountManager).getAccount(0L);
	}

	@Test
	public void nonExistentBeneficiaryDetails() throws Exception {
		Account account = new Account("5645363735", "Crystal Methyd");
	...
	public void addBeneficiary() throws Exception {
		Account account = new Account("5645363735", "Crystal Methyd");
		Beneficiary beneficiary = new Beneficiary("Anouk Pancaldi", new Percentage(0.2));

		given(accountManager.getAccount(0L)).willReturn(account);

		mockMvc.perform(post("/accounts/{accountId}/beneficiaries", 0L)
						.contentType(MediaType.APPLICATION_JSON)
						.content(asJsonString(beneficiary)))
				.andExpect(status().isCreated());
//				.andExpect(header().string("Location", "/accounts/0/beneficiaries/1"));

		verify(accountManager).addBeneficiary(0L, asJsonString(beneficiary));
	}
	...
	public void removeBeneficiary() throws Exception {
		...
		mockMvc.perform(delete("/accounts/{accountId}/beneficiaries/{beneficiaryId}", 0L, "Anouk Pancaldi"))
				.andExpect(status().isNoContent());

		verify(accountManager).removeBeneficiary(0L, "Anouk Pancaldi", Map.of());
	}
